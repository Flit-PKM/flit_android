...
- **Onboarding + welcome note:** `AppConfig.ONBOARDING_REVISION` gates onboarding display (`SettingsRepository.shouldShowOnboarding`). The welcome note is sourced from `res/raw/welcome_note.md` and seeded as note id `0` only once at install-time when the notes table is empty; it is never auto-recreated later if missing. The first-run tour (`OnboardingOverlay`) walks the bottom bar, optional model pick, note options on Home (copy + highlighted welcome card; same actions as long-press or (⋮) on the note screen), title + body highlights on the note screen, border-only pulse styling aligned with Settings tour sections, menu and search on Home, editing title (app bar) and body on the note screen with highlights on the editor and Categories/Relationships cards, then the Categories management screen, then Settings; coachmarks include **Skip** (same completion as Finish) on every step including Settings.
